...
public class Mainmenu {
    ...
    List<kiosk.Foodmenu> pastaMenu = new ArrayList<>();

    List<kiosk.Foodmenu> sideMenu = new ArrayList<>();

    List<kiosk.Foodmenu> drinkMenu = new ArrayList<>();

    kiosk.Order order = new Order();

    Scanner sc = new Scanner(System.in);

    public Mainmenu() { //각각의 리스트에 이름, 가격, 음식설명 추가
        pastaMenu.add(new kiosk.Foodmenu("알리오 올리오", "6.9", "풍부한 마늘향을 끌어올린 가장 기본적인 오일파스타"));
        pastaMenu.add(new kiosk.Foodmenu("토마토 파스타", "8.9", "그라냐 파다노 치즈를 듬뿍 갈아올린 토마토 파스타"));
        pastaMenu.add(new kiosk.Foodmenu("봉골레 파스타", "9.9", "조개의 시원한 맛과 함께 마늘의 알싸함이 감도는 파스타"));

        sideMenu.add(new kiosk.Foodmenu("고르곤 졸라피자", "8.9", "고르곤졸라 치즈로 만든 피자를 달콤한 꿀과 함께 즐기는 피자"));
        sideMenu.add(new kiosk.Foodmenu("스파이시 윙&봉", "7.9", "특제 바베큐 양념을 덧발라 구운 매운 윙&봉"));
        sideMenu.add(new kiosk.Foodmenu("갈릭 바게트", "4.9", "진한 마늘향과 버터를 발라 바삭하게 구운 마늘바게트"));

        drinkMenu.add(new kiosk.Foodmenu("탄산음료", "2.0", "콜라,사;이다,환타"));
        drinkMenu.add(new kiosk.Foodmenu("레몬에이드", "4.0", "상큼한 레몬이 가득 들어간 레몬에이드"));
        drinkMenu.add(new kiosk.Foodmenu("자몽에이드", "4.0", "상큼한 자몽슬라이스가 통으로 들어간 자몽에이드"));
    ...
    public void ShowpastaMenu() {
        ...
        for (int i = 0; i < pastaMenu.size(); i++) { //pastaMenu리스트에 있는 size만큼 반복 (size는 3이므로 i= 0,1,2)
            kiosk.Foodmenu menu = pastaMenu.get(i);  // pastaMenu리스트에서 get()을 사용하여 menu안에 버거정보를 대입
        ...
        kiosk.Foodmenu selectedpasta = pastaMenu.get(pastaInfo);  //Foodmenu 객체를 만들어서 pastainfo 정보대입
    ...
    public void ShowsideMenu() {
        ...
        for (int i = 0; i < sideMenu.size(); i++) { //sideMenu리스트에 있는 size만큼 반복 (size는 3이므로 i= 0,1,2)
            kiosk.Foodmenu menu = sideMenu.get(i);  // sideMenu리스트에서 get()을 사용하여 menu안에 버거정보를 대입
            System.out.println((i + 1) + ". " + menu.getName() + "  | " + menu.getPrice() + "  | " + menu.getDescription());
        } // (i + 1)을 사용한 이유는 i는 배열의 첫번째 값부터 출력하기 위해 0을 사용하였는데, 번호는 1번부터 출력해야하니 i+1 을 해줘서 1,2,3이 출력되게 만들어 줬음.
        ...
        kiosk.Foodmenu selectedside = sideMenu.get(sideInfo);  //Foodmenu 객체를 만들어서 sideinfo 정보대입
    ...
    public void ShowdrinkMenu() {
        ...
        for (int i = 0; i < drinkMenu.size(); i++) { //drinkMenu리스트에 있는 size만큼 반복 (size는 3이므로 i= 0,1,2)
            kiosk.Foodmenu menu = drinkMenu.get(i);  // drinkMenu리스트에서 get()을 사용하여 menu안에 버거정보를 대입
            System.out.println((i + 1) + ". " + menu.getName() + "  | " + menu.getPrice() + "  | " + menu.getDescription());
        } // (i + 1)을 사용한 이유는 i는 배열의 첫번째 값부터 출력하기 위해 0을 사용하였는데, 번호는 1번부터 출력해야하니 i+1 을 해줘서 1,2,3이 출력되게 만들어 줬음.
        ...
        kiosk.Foodmenu selecteddrink = drinkMenu.get(drinkInfo); //Foodmenu 객체를 만들어서 sideinfo 정보대입
    ...
    public void ShowOrderMenu() {
        ...
        List<kiosk.Foodmenu> basket = order.getbasket();
